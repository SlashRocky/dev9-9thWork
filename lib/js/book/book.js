        ...
        for(var i =　0; i　<　10; i++){
          ...
          var BookTag =  '<li>' +
              ...
              '<p class="text">' + BookDescript +'</p>';
					...
					//ログインしてたら　（input[name=userId]に値が入っているなら）
          if(　$("input[name=userId]").val()　){
            BookTag += '<button class="memoBtn" type="button" data-id="'+BookId+'" name="button"><span class="icon-pencil"></span>保存する</button>';
          }
          //共通
          BookTag += '</div></div></li>';
          $('.searchResult').find('ul').append(BookTag);
